...
model = dict(
    ...
    test_cfg=dict(score_thr=0.001, nms=dict(type="nms", iou_threshold=0.2)),
)
...
train_pipeline = [
    dict(type="Mosaic", img_scale=img_scale, pad_val=192.0),
    dict(
        type="RandomAffine",
    ...
    dict(type="MixUp", img_scale=img_scale, ratio_range=(0.8, 1.6), pad_val=192.0),
    ...
    dict(
        ...
        pad_val=dict(img=(192.0, 192.0, 192.0)),
    ),
...
test_pipeline = [
    dict(type="LoadImageFromFile", backend_args=backend_args),
    dict(type="Resize", scale=img_scale, keep_ratio=True),
    dict(type="Pad", pad_to_square=True, pad_val=dict(img=(192.0, 192.0, 192.0))),
    dict(type="LoadAnnotations", with_bbox=True),
    dict(
        type="PackDetInputs",
